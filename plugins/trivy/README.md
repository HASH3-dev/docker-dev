# trivy

Este é o plugin pai de segurança de dependências. Ele instala o Trivy e fornece
a interface estável para seus adaptadores:

```bash
docker-dev-trivy-gate scan-lock <lockfile>
docker-dev-trivy-gate scan-path <path>
```

`scan-lock` deve ser chamado pelo adaptador depois de gerar uma resolução
temporária, completa e sem scripts de pacote. Se o gate retornar código diferente
de zero, o adaptador não pode executar a instalação real.

Os subplugins habilitados ficam em `plugins.enabled`. O subplugin `npm` é o
referência inicial; futuros adaptadores podem implementar pnpm, Yarn, pip e uv
sem precisar conhecer a instalação ou as opções do Trivy.

O manifesto `plugin.yml` usa o schema `docker-dev/plugin/v1`; subplugins usam
o schema separado `docker-dev/subplugin/v1`. Ambos são validados pelo dispatcher
antes de resolver comandos habilitados.
