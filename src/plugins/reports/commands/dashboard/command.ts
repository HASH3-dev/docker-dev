...
function renderValue(key: string, value: unknown): string {
  ...
  return typeof value === "string"
    ? `<div class="markdown" data-markdown>${escapeHtml(text)}</div>`
...
</style></head><body><main><h1>Security reports</h1><p class="intro">${reports.length} relatório${reports.length === 1 ? "" : "s"} disponível${reports.length === 1 ? "" : "is"} · ${findings.length} achado${findings.length === 1 ? "" : "s"}</p><div class="summary">${counts.map(([level, count]) => `<div class="metric"><span>${level}</span><b>${count}</b></div>`).join("")}</div>${content}</main><script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script><script src="https://cdn.jsdelivr.net/npm/dompurify/dist/purify.min.js"></script><script>document.querySelectorAll('.tab').forEach(tab=>tab.addEventListener('click',()=>{const index=tab.dataset.tab;document.querySelectorAll('.tab').forEach(item=>{const active=item===tab;item.classList.toggle('active',active);item.setAttribute('aria-selected',String(active))});document.querySelectorAll('[role="tabpanel"]').forEach(panel=>panel.hidden=panel.id!=='report-'+index)}));document.querySelectorAll('.secret button').forEach(button=>button.addEventListener('click',()=>{const value=button.nextElementSibling;value.hidden=!value.hidden;button.setAttribute('aria-label',value.hidden?'Mostrar valor':'Ocultar valor')}));document.querySelectorAll('[data-markdown]').forEach(element=>element.innerHTML=DOMPurify.sanitize(marked.parse(element.textContent)))</script></body></html>`;
}
